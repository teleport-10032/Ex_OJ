...
$page_size = 10;
$sql = "select count(*) as amount from problem";
            ...
            <article id="wrapper">
                <nav class="navbar navbar-default" role="navigation"
                    ...
                    <div>
                        <ul class="nav navbar-nav">
                            <li>
                                <h3>&nbsp;&nbsp;Problem List</h3> <a href="/F.A.Q/test_case_upload" target="_blank" style="color: dodgerblue;">如何上传测试样例?</a>
                            </li>
                        </ul>
                ...
                <div class="row clearfix">
                    <div class="col-md-12 column">
                        <table class="table table-striped table-hover table-responsive">
                            ...
                            $str = "select problem_id,problem_title,user_name,problem_create_time,problem_visible,problem_testcase_path from user,problem where user_id=problem_author order by problem_id desc limit " . ($page - 1) * $page_size . "," . $page_size;
                            $result = $conn->query($str);
                            while (list($problem_id, $problem_title, $problem_author,$problem_create_time,$problem_visible,$problem_testcase_path) = $result->fetch_row()) {
                                ...
                                if($problem_testcase_path != '')
                                {
                                    $file = iconv("UTF-8", "GBK","../../judger/test_case/".$problem_id.'/test_case.zip');
                                    echo "
                ...
                <div class="pagination pagination-right" id="page">
                    <ul class="pagination">
                        ...
                        $nex = $page + 1;
                        if ($page != 1)
                            echo "<li> <a href=/admin/problem_admin?page=$pre><</a></li>";
                        else
                            echo "<li> <a href =\"javascript:return false;\"> <i style=\"opacity: 0.2\"><</i></a></li>";
                        for ($i = 1; $i <= $page_count; $i++)
                            if ($i == $page)
                                echo "<li> <a href=/admin/problem_admin?page=$i><b><i>$i</i></b></a></li>";
                            else
                                echo "<li> <a href=/admin/problem_admin?page=$i>$i</a></li>";
                        if ($page != $page_count)
                            echo "<li> <a href=/admin/problem_admin?page=$nex>></a></li>";
                        else
                            echo "<li> <a href =\"javascript:return false;\"> <i style=\"opacity: 0.2\">></i></a></li>";
                        ?>
                    </ul>
